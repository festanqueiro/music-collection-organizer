// The live stream a stream-mode cast session plays. The renderer records
// MCO's own output with MediaRecorder and pushes WebM chunks here; ffmpeg
// re-encodes them into a rolling HLS playlist (H.264 + AAC in MPEG-TS,
// the most widely supported live format on Cast TVs) in a temp folder,
// which a small HTTP server exposes on the LAN for the TV to pull.
// (Speakers always cast in direct mode, so there's no audio-only stream.)
...
// The TV starts playing a little behind the live edge, so there's no
// point loading it before there's something to buffer.
const SEGMENTS_BEFORE_READY = 2

const INPUT_ARGS = ['-hide_banner', '-loglevel', 'error', '-fflags', '+genpts', '-f', 'matroska', '-i', 'pipe:0']

export function buildFfmpegArgs(outputDir: string): string[] {
  return [
    ...INPUT_ARGS,
    '-map', '0:v:0',
    '-map', '0:a:0',
...
export class CastStream {
  ...
  readonly contentType = 'application/x-mpegurl'

  async start(): Promise<void> {
    const ffmpegPath = resolveFfmpegPath()
    if (!ffmpegPath) throw new Error('ffmpeg is not available')
    const ffmpeg = spawn(ffmpegPath, buildFfmpegArgs(this.dir), { stdio: ['pipe', 'ignore', 'pipe'] })
    this.ffmpeg = ffmpeg
    ...
    this.server = createServer((req, res) => {
      ...
      this.serveHlsFile(name, req.method, res)
    })
  ...
  url(localAddress: string): string {
    ...
    return `http://${localAddress}:${port}/${this.token}/${PLAYLIST_NAME}`
  }
